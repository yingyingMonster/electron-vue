<template v-slot:spec>
  <div class="discountContent" v-loading="loading">
    <div class="dialogTitle justifyS">
      <span
        >折扣<i class="el-icon-minus"></i
        >{{ this.dialogVal.info ? this.dialogVal.info.name : "批量折扣" }}</span
      >
      <i class="el-icon-close" @click="close"></i>
    </div>
    <div class="discount-box justifyS">
      <span class="discountLab" @click="toggleEvent">折扣/减免</span>
      <el-input
        v-model="discountAmount"
        placeholder="请输入打折额度"
        type="number"
        onkeyup="this.value = this.value.replace(/[^\d.]/g,'')"
        oninput="if(value>99)value=99"
      >
        <span slot="suffix" class="suffix-icon">%</span>
      </el-input>
    </div>

    <div class="discount-total justifyS">
      <transition name="fade">
        <div v-show="discountAmount">
          打折后<span>￥{{ totalprice }}</span>
        </div>
      </transition>
      <transition name="fade">
        <div v-show="discountAmount">包含小料</div>
      </transition>
    </div>
    <div class="fixed-count">
      <span
        v-for="(item, index) in discountList"
        :key="index"
        @click="checkedFixDiscount(index, item)"
        :class="index == discountIndex ? 'active' : ''"
        >{{ item }}%</span
      >
    </div>

    <div class="keyboard">
      <table class="numBtn">
        <tbody>
          <tr>
            <td>
              <el-button @click="getNum(1)">1</el-button>
            </td>
            <td>
              <el-button @click="getNum(2)">2</el-button>
            </td>
            <td>
              <el-button @click="getNum(3)">3</el-button>
            </td>
            <td rowspan="2">
              <el-button class="funBtn" @click="delBtn">删除</el-button>
            </td>
          </tr>
          <tr>
            <td>
              <el-button @click="getNum(4)">4</el-button>
            </td>
            <td>
              <el-button @click="getNum(5)">5</el-button>
            </td>
            <td>
              <el-button @click="getNum(6)">6</el-button>
            </td>
          </tr>
          <tr>
            <td>
              <el-button @click="getNum(7)">7</el-button>
            </td>
            <td>
              <el-button @click="getNum(8)">8</el-button>
            </td>
            <td>
              <el-button @click="getNum(9)">9</el-button>
            </td>
            <td rowspan="2">
              <el-button class="funBtn" @click="subBtn">
                <img src="@/assets/delBtn.png" alt="" />
              </el-button>
            </td>
          </tr>
          <tr>
            <td>
              <el-button></el-button>
            </td>
            <td>
              <el-button @click="getNum(0)">0</el-button>
            </td>
            <td>
              <el-button></el-button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>

    <div class="fixed-reason">
      <span class="title">打折原因</span>
      <transition name="fade">
        <div class="reasonBox justify">
          <span
            v-for="(item, index) in fixedReason"
            :key="index"
            @click="checkReason(index, item)"
            :class="remarksIndex == index ? 'active' : ''"
            >{{ item }}</span
          >
        </div>
      </transition>
    </div>

    <el-input
      type="textarea"
      class="reason"
      v-model="remarks"
      @input="remarksChange"
      placeholder="请输入打折原因"
    ></el-input>

    <el-button type="darkGreen" class="confirmBtn" @click="confirmBtn"
      >确认</el-button
    >
  </div>
</template>
<script>
import { getDiscount } from "@/api";
export default {
  props: ["dialogVal"],
  data() {
    return {
      // 是否显示折扣计算价格
      discountAmount: "",
      // 备注信息
      remarks: "",
      remarksIndex: -1,
      // 折扣价格列表
      discountList: "",
      // 折扣列表下标
      discountIndex: -1,
      // 深拷贝后的商品列表以及商品价格
      list: "",
      listCopy: "",
      // 基础价格数组
      sumBase: [],
      // 小料价格数组
      mater: [],
      //订单总价
      totalprice: 0,
      // 固定折扣原因
      fixedReason: [],
      loading: true,
    };
  },
  methods: {
    // 数字键盘点击事件
    getNum(value) {
      console.log(value);
      if (parseInt(this.discountAmount + value) < 100) {
        this.discountAmount += value;
        this.discountIndex = -1;
      }
    },
    // 数字键盘-删除
    delBtn() {
      this.discountAmount = "";
    },
    // 数字键盘-删减
    subBtn() {
      let count = this.discountAmount;
      this.discountAmount = count.toString().substring(0, count.length - 1);
    },
    // 选择固定折扣
    checkedFixDiscount(index, val) {
      this.discountIndex = index;
      this.discountAmount = val.toString();
    },
    // 确认按钮
    confirmBtn() {
      for (let item of this.listCopy) {
        item.amount = null;
      }
      this.$store.commit("delOrder", this.listCopy);
      for (let item of this.list) {
        item.discountRemark = this.remarks;
        item.isDeduction = false;
        item.deductionAmount = 0;
        item.deductionRemark = "";
        this.$store.commit("createOrder", item);
      }
      //置空批量列表
      this.$parent.clearBatch();
      // 关闭弹窗
      this.$parent.closePopover();
    },
    // 选择固定原因
    checkReason(index, item) {
      this.remarksIndex = index;
      this.remarks = item;
    },
    // 监听input value
    remarksChange(e) {
      console.log(e);
      if (this.fixedReason[this.remarksIndex] != e) {
        this.remarksIndex = -1;
      }
    },
    // 关闭按钮
    close() {
      // 关闭弹窗
      this.$parent.closePopover();
    },
    toggleEvent() {
      this.$notify({
        title: "提示",
        message: "不支持批量减免",
        type: "warning",
      });
    },
  },
  mounted() {
    // 深拷贝商品信息,避免对原数组产生影响
    this.list = this._.cloneDeep(this.dialogVal);
    this.listCopy = this._.cloneDeep(this.dialogVal);
    console.log(this.dialogVal);
    // 获取折扣列表
    getDiscount()
      .then((res) => {
        this.loading = false;
        console.log(res);
        this.discountList = res.data.discountList.map((item) => {
          return parseInt(item * 100);
        });
        this.fixedReason = res.data.reasonList;
      })
      .catch((err) => {
        this.loading = false;
        console.log(err);
      });
  },
  watch: {
    // 监听折扣
    discountAmount: function (newVal) {
      const total = [];
      for (let item of this.list) {
        const sumBase = [];
        const sumMater = [];
        for (let it of item.specList) {
          for (let ie of it.specItemList) {
            if (it.checkedId == ie.id) {
              sumBase.push(ie.basePrice);
              sumMater.push(ie.price);
            }
          }
        }
        // 修改折扣值
        item.discountValue = this._.round(newVal / 100, 2);
        const price = item.price;
        const discountValue = item.discountValue;
        // 修改折扣价格
        item.discountPrice = this._.round(
          price +
            this._.sum(sumBase) -
            (price + this._.sum(sumBase)) * discountValue,
          2
        );
        // 修改每个商品价格
        item.showPrice = this._.round(
          (price + this._.sum(sumBase)) * discountValue + this._.sum(sumMater),
          2
        );
        // 商品是否打折是否减免
        item.isDiscount = true;
        // 计算总价
        total.push(item.showPrice * item.amount);
        this.totalprice = this._.round(this._.sum(total), 2);
      }
    },
  },
};
</script>
<style lang="scss">
.discount-box .el-input {
  width: 50%;
  input {
    border: none;
    text-align: right;
    color: #4f424c;
  }
  input::-webkit-outer-spin-button,
  input::-webkit-inner-spin-button {
    -webkit-appearance: none;
  }
  input[type="number"] {
    -moz-appearance: textfield;
  }
}
.numBtn button {
  height: 42px;
}
.content .el-textarea__inner {
  color: #4f424c;
  background: #f5f5f5;
}
</style>
<style lang="scss" scoped>
.discountContent {
  height: 100%;
  width: 100%;
  padding: 10px;
  box-sizing: border-box;
  position: relative;
}
.dialogTitle {
  width: 100%;
  height: 60px;
  align-items: center;
  padding: 0 10px;
  box-sizing: border-box;
  i {
    cursor: pointer;
  }
}
.discount-box {
  align-items: center;
  width: 100%;
  height: 50px;
  border: 1px solid #3bc697;
  border-radius: 4px;
  padding: 0 10px;
  box-sizing: border-box;
  background: #fafffd;
}
.discountLab {
  width: 80px;
  height: 35px;
  line-height: 35px;
  text-align: center;
  background: #009e69;
  font-size: 12px;
  border-radius: 10px;
  color: #fff;
  cursor: pointer;
}
.suffix-icon {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 100%;
}
.confirmBtn {
  border-radius: 10px;
  position: absolute;
  left: 0;
  bottom: 10px;
  width: calc(100% - 20px);
  margin: 10px;
}
.fixed-count {
  display: flex;
  justify-content: flex-start;
  align-items: center;
  margin: 10px 0 20px 0;
  span {
    display: inline-block;
    width: 23%;
    margin: 0 1%;
    background: #f5f5f5;
    color: #4f424c;
    border-radius: 10px;
    font-size: 12px;
    text-align: center;
    height: 35px;
    line-height: 35px;
    cursor: pointer;
  }
}
.numBtn {
  width: 100%;
  td {
    text-align: center;
    width: 25%;
  }
  button {
    width: 98%;
    text-align: center;
    background: #f5f5f5;
    border-radius: 10px;
    margin: 2px 0;
  }
}
.funBtn {
  height: 92px;
}
.discount-total {
  color: #4f424c;
  font-size: 13px;
  width: 100%;
  height: 30px;
  line-height: 30px;
  span {
    color: #f06543;
    font-size: 15px;
  }
}
.fixed-reason {
  color: #4f424c;
  font-size: 14px;
  margin: 20px 0;
}
.title {
  display: inline-block;
  font-size: 15px;
}
.reasonBox {
  display: flex;
  justify-content: flex-start;
  align-items: center;
  margin: 10px 0;
  span {
    display: inline-block;
    width: 31%;
    margin: 0 1%;
    background: #f5f5f5;
    color: #4f424c;
    border-radius: 10px;
    font-size: 12px;
    text-align: center;
    height: 35px;
    line-height: 35px;
    cursor: pointer;
  }
}
.active {
  background: #009e69 !important;
  color: #fff !important;
}
</style>